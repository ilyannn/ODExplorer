...
@interface HelpersTests : XCTestCase <ODLazyMutableArrayDelegate>
...
- (void)testLazyMutableCorrectness {
    ODLazyMutableArray *lazy = [[ODLazyMutableArray alloc] initWithDelegate:self contents:@[@0, @10, @20]];
...
- (void)testLazyMutableMemorySemantics {
    ...
    NSMutableArray * lazy = // [NSMutableArray new];
                            [[ODLazyMutableArray alloc] initWithDelegate:self];
...
- (void)testDates {
    for (ODDateFormatter *fmt in @[[ODJSONDateFormatter new], [ODISO8601DateFormatter new]]) {
        XCTAssert([fmt testDate:[NSDate new]], @"Writing and reading back the date using %@", fmt);
    }
}


- (void)testThreadSafeFormatters {
    NSOperationQueue *queue = [NSOperationQueue new];
    
    for (ODDateFormatter *fmt in @[[ODJSONDateFormatter new], [ODISO8601DateFormatter new]]) {
        for (NSUInteger index = 0; index < 100; index++) {
            [queue addOperationWithBlock:^{
                XCTAssert([fmt testDate:[NSDate new]],
                          @"Writing and reading back the date using %@, from thread %@",
                          fmt, [NSThread currentThread]);
            }];
        }
    }
}
